...
//int Amt[Tipos] = [1200,3200,3200]; //Alcance maximo teorico
int Pt[Tipos] = ...;
int Srx[Tipos] = ...; //Sensibilidade rx
int f[Tipos] = ...; //Frequencia (MHz)
float It[Tipos] = ...; //Corrente consumida (DC=1%,payload=16B)
float Mt[Tipos] = ...; //Custo medio em dolar
int Gt[Tipos] = ...; //Ganho tx antena
int Gr[Tipos] = ...; //Ganho rx antena
...
execute VIZINHANCA {
  ...
  var Pr;
  //A vizinhanca eh preenchida de acordo com a potencia de recepcao calculada, mediante distancia entre dois nos. Eh considerada a sensibilidade dos possiveis tipos de vizinhos. Se a potencia de recepcao calculada for maior que a sensibilidade do receptor, entao sao vizinhos.
  for(var t in Tipos){
	  for(var i in V){
	    for(var j in V){
	      	if(i != j){
		      	
		      	Pr = (Pt[t]+Gt[t]+Gr[t]+27.56-(20*Opl.log(f[t]))-(20*Opl.log(D[i][j])));
