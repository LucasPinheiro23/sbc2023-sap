	  ...
	  for(var i in V){
	    for(var j in V){
	      	if(i != j){
		      	  ...
		      	  if(D[i][j] <= Rmax[t] && D[i][j] >= 0.8*Rmax[t]){
		      	    N[t][i].add(j);
		      	  }
...
dexpr float C = sum(i in V) sum(t in Tipos) 3.14159265358979323846 * pow(Rmax[t],2)*s[t][i];
//dexpr float C_alt = sum(i in V, t in Tipos, u in Tipos, j in N[t][i]) s[u][j];
