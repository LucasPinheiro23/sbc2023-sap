...
from efficient_sap_sbc_abstract_model_weightedsum import *
...
for L in range(10, 15, 5):
    for d in range(1, 2):
        for a in range(50, 75, 25):
            ...
            sys.stdout = open(
                "./output/logs/" + instance_filename[:-3] + instance_a + ".txt", "w"
            )
            ...
            min_fo = {
                "E": preproc_E_min(instance1),
                "C": get_fo_min(
                    model,
                    {"E": 0, "C": 1, "M": 0},
                    instance_path + instance_filename,
                    solver,
                    solver_exec,
                )["C"],
            }
            ...
            # Se nao, significa que o problema eh multiobjetivo (ponderado por alphas)
                ...
                model.weig = (
                    alpha["E"] * fo["E"] * model.exp_norm_E
                    - alpha["C"] * fo["C"] * model.exp_norm_C
                    + alpha["M"] * fo["M"] * model.exp_norm_M
                )
            ...
            if fo["E"] == 1:
                print(
                    "E: "
                    + str(value(instance.norm_E))
                    + " -- "
                    + str(
                        min_fo["E"]
                        + (max_fo["E"] - min_fo["E"]) * value(instance.norm_E)
                    )
                    + " mA"
                )
            ...
            if fo["C"] == 1:
                print(
                    "C: "
                    + str(value(instance.norm_C))
                    + " -- "
                    + str(
                        min_fo["C"]
                        + (max_fo["C"] - min_fo["C"]) * value(instance.norm_C)
                    )
                    + " points"
                )
            ...
            if fo["M"] == 1:
                print(
                    "M: "
                    + str(value(instance.norm_M))
                    + " -- $"
                    + str(
                        min_fo["M"]
                        + (max_fo["M"] - min_fo["M"]) * value(instance.norm_M)
                    )
                )
